...
The venv is **required**, not a preference: Homebrew's Python is externally managed, so `pip3 install -r requirements.txt` aborts with `externally-managed-environment` (PEP 668), and `/opt/homebrew/bin/python3` has no PyQt6 of its own. `ffmpeg`/`ffprobe` must be on Homebrew (`/opt/homebrew/bin`). yt-dlp runs as the venv's `yt_dlp` *package*, never a system binary, and must stay current or downloads 403 — follow `.claude/skills/check-ytdlp/SKILL.md` to check, diagnose or upgrade it.
...
To launch, screenshot or smoke-test the app without a display, use the `run-autonotes` skill (`.agents/skills/run-autonotes/`) rather than driving Qt by hand.
...
Stages in order:
1. **Download** (`pipeline/downloader.py`) — wraps `yt-dlp` for YouTube URLs; local files are used directly. Teams/SharePoint downloads (`pipeline/teams_downloader.py`) use browser cookies (`--cookies-from-browser chrome`, then `edge`) to authenticate. `ffmpeg` is assumed at `/opt/homebrew/bin`. When no VTT/SRT lands, `pipeline/sharepoint_transcript.py` fetches the Teams transcript from SharePoint's Stream API — yt-dlp's SharePoint extractor never populates `subtitles`, so `--write-sub` is a no-op for `stream.aspx` URLs. It scrapes the page's `g_fileInfo` blob for `.spItemUrl` (site path / drive / item — note site collections may be `/teams/`, not just `/sites/` or `/personal/`) plus a `hasTranscripts` flag, calls `items/{id}?$expand=media/transcripts` for a pre-signed `temporaryDownloadUrl`, then reads it with `format=json`. Entries already carry `speakerDisplayName`, so a transcribed recording skips Whisper *and* diarization. Reuses the same cookies as the download — no Azure app registration needed. This is the only Teams transcript source; the Graph client deliberately does not fetch transcripts (so it never needs `OnlineMeetingTranscript.Read.All`) and supplies only attendees, the AI recap and the title. Any failure returns `None` and the pipeline transcribes normally.
...
After each transcription the segments are saved to `~/Library/Logs/AutoNotes/last_transcript.json` (with video title + timestamp). A "Reuse last transcript" checkbox (enabled only when that file exists) skips audio extraction, Whisper, and diarization on the next run; the worker warns in the log if the saved transcript came from a different video. A transcript that came with the download (Teams VTT / SharePoint Stream API) still takes priority over the saved one.
